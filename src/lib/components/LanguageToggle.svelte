<script lang="ts">
	import { localeStore, setLocale, getLocale } from '$lib/i18n';
	...
	function toggleLanguage() {
		setLocale(currentLocale === 'pl' ? 'en' : 'pl');
	}
</script>
...
>
	...
	<span class="hidden sm:inline">{currentLocale === 'pl' ? 'EN' : 'PL'}</span>
	<span class="inline sm:hidden">{currentLocale === 'pl' ? 'EN' : 'PL'}</span>
</button>
