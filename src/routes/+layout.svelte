		...
		<div class="grid md:grid-cols-4 gap-12 mb-12">
			<div>
				...
				<p class="text-gray-200 leading-relaxed">
					{$tSafe('footer.description')}
				</p>
			</div>
			<div>
				<h4 class="text-sm uppercase tracking-wider mb-4 font-bold text-[#FB7185]">{$tSafe('footer.offer')}</h4>
				<ul class="space-y-2 text-gray-200 text-sm">
					<li>• {$tSafe('footer.services.interior_design')}</li>
					<li>• {$tSafe('footer.services.space_arrangement')}</li>
					<li>• {$tSafe('footer.services.visual_identity')}</li>
					<li>• {$tSafe('footer.services.computer_graphics')}</li>
					<li>• {$tSafe('footer.services.ai_technology')}</li>
				</ul>
			</div>
			<div>
				<h4 class="text-sm uppercase tracking-wider mb-4 font-bold text-[#FB7185]">{$tSafe('footer.clients')}</h4>
				<ul class="space-y-2 text-gray-200 text-sm">
					<li>• {$tSafe('footer.clientTypes.apartment_buyers')}</li>
					<li>• {$tSafe('footer.clientTypes.developers')}</li>
					<li>• {$tSafe('footer.clientTypes.real_estate')}</li>
					<li>• {$tSafe('footer.clientTypes.startups')}</li>
					<li>• {$tSafe('footer.clientTypes.commercial')}</li>
				</ul>
			</div>
			<div>
				<h4 class="text-sm uppercase tracking-wider mb-4 font-bold text-[#FB7185]">{$tSafe('footer.contact')}</h4>
				<ul class="space-y-2 text-gray-200 font-medium">
					<li>kontakt@pikastro.eu</li>
					<li>{$tSafe('footer.location')}</li>
					<li class="pt-2">
						<a href="/o-mnie" class="text-[#FB7185] hover:text-white transition-colors">{$tSafe('nav.about')}</a> •
						<a href="/#oferta" class="text-[#FB7185] hover:text-white transition-colors">{$tSafe('nav.offer')}</a>
					</li>
				</ul>
			</div>
		</div>
		<div class="pt-8 border-t border-white/20 text-center text-sm text-gray-300">
			<p class="font-medium">&copy; 2025 Pikastro. {$tSafe('footer.copyright')}</p>
		</div>
	</div>
</footer>
